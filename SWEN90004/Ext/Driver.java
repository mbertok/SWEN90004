...
public class Driver {
	...
	/**
	 * Simulates the immigration phase Adds the immigrants in the map if there
	 * are spaces
	 */
	...
	public void Interaction() {
		...
		for (int[] i : Map.getAgents()) {
			for (Agent neighbor : Map.findNeighbours(i[0], i[1])) {
	...
	public void Reproduction() {
		...
		try {
			...
			for (int[] i : coord) {
				...
				// System.out.println("["+i[0]+"]["+i[1]+"] Agent ptr
				// after"+a.getPtr());
				// check if ready to reproduce
				if (a.check(a.getPtr())) {
					// selecting cell to put reproduce in
					List<int[]> neighboringSpaces = Map.findNeighboringSpaces(i[0], i[1]);
					if (neighboringSpaces.size() > 0) {
						int[] point = neighboringSpaces.get(r.nextInt(neighboringSpaces.size()));
						Map.addAgent(a.reproduce(Map.getMutationRate(), initialPtr, 0, noOfEthnicities), point[0],
								point[1]);
	...
	public void PrintCount() {
		System.out.println("\t\t\t\tCC:" + Map.getCCcount() + ";CD:" + Map.getCDcount() + "DC:" + Map.getDCcount()
				+ ";DD:" + Map.getDDcount());
	}
	...
	/**
	 * Prepares a string to write in the file.
	 * 
	 * @return
	 */
